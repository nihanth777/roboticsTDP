...
%goal keeper has ball blue team
elseif ball_posession == 1 && index == 5   
       %% to to centre of goal area and kicks ball to nearest player
    [px,py,~,~,~] = playerPosition(index,players,ball);
    tx = x_gk_away_init;
    ty = ylimit_outer/2;
...
elseif ball_posession == 0 && index == 1
  ...
  if(bx<80)
        ...
        [players]=target(index,players,ball,bx,by);
        [ball] = dribble(players,ball,index);
...
elseif ball_posession == 0 && index == 7 %%%% stand in defending position
    ...
    [px,py,~,~,~] = playerPosition(index,players,ball);
    [players]=target(index,players,ball,px-10,py-10);

elseif ball_posession == 0 && index == 8
    
    [px,py,bx,by,~] = playerPosition(index,players,ball);
    [players]=target(index,players,ball,bx-5,by-10);
