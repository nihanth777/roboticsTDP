...
elseif ball_posession1 == 1 && index == 2
    ...
    elseif ball_posession1 == 1 && index == 4
        ...
        [players,ball] = target(index,players,ball,px,py);% %GOAL POST almost neart penalty and then kick POSITION HERE
...
if  ball_posession1 == 1 && index == 8
    ...
    tx = xlimit_outer-xlimit_boarderstrip-2*penaltymark;    %C
    ty = ylimit_outer/2;    
    if (197<=px)
...
elseif ball_posession1 == 1 && index == 7
    [px,py,bx,by,~] = playerPosition(index,players,ball);
    [players,ball] = target(index,players,ball,272,100);% rand here        %%%C
    [players,ball] = dribble(players,ball,index);
